...
static inline float v4_hc_sigmoid(float x) {
    if (x >= 0.0f) return 1.0f / (1.0f + expf(-x));
    float z = expf(x);
    return z / (1.0f + z);
}

static inline int v4_hc_forward(const V4HyperConnection *model, const float *input,
    ...
    for (int token = 0; token < n_tokens; token++) {
        ...
        for (int h = 0; h < streams; h++) {
            pre[h] = v4_hc_sigmoid(mixes[h] * model->scale[0] + model->base[h]) + model->hc_eps;
            post[(int64_t)token * streams + h] = 2.0f * v4_hc_sigmoid(

#ifndef DEEPSEEK_V4_ATTENTION_H
#define DEEPSEEK_V4_ATTENTION_H
...
typedef struct {
    ...
    const float *o_a;
    const float *o_b;
} DeepSeekV4SlidingAttentionF32;

static inline void deepseek_v4_attn_matvec(float *output, const float *weight,
...
static inline void deepseek_v4_attn_rmsnorm(float *output, const float *input,
...
static inline void deepseek_v4_attn_rope(float *vector, int head_dim, int rope_dim,
...
static inline int deepseek_v4_sliding_attention_cached_f32(
        const DeepSeekV4SlidingAttentionF32 *model, DeepSeekV4KVCacheF32 *cache,
    ...
    if (!model || !cache || !input || !output || n_tokens < 0 || start_position < 0 ||
        ...
        cache->mode != DEEPSEEK_V4_KV_CACHE_RING || cache->capacity != model->window ||
        cache->head_dim != model->head_dim)
        return 0;
    if (cache->count > 0 &&
        start_position != deepseek_v4_kv_cache_position(cache, cache->count - 1) + 1)
        return 0;
    ...
    for (int token = 0; token < n_tokens; token++) {
        ...
        deepseek_v4_attn_matvec(q_a, model->q_a, x, model->q_rank, hidden);
        deepseek_v4_attn_rmsnorm(q_norm, q_a, model->q_a_norm, model->q_rank, model->norm_eps);
        deepseek_v4_attn_matvec(query, model->q_b, q_norm, q_width, model->q_rank);
        for (int head = 0; head < heads; head++) {
            float *row = query + head * head_dim;
            deepseek_v4_attn_rmsnorm(row, row, NULL, head_dim, model->norm_eps);
            deepseek_v4_attn_rope(row, head_dim, model->rope_dim, position, model->rope_theta, 0);
        }
        deepseek_v4_attn_matvec(kv_raw, model->kv, x, head_dim, hidden);
        deepseek_v4_attn_rmsnorm(key, kv_raw, model->kv_norm, head_dim, model->norm_eps);
        deepseek_v4_attn_rope(key, head_dim, model->rope_dim, position, model->rope_theta, 0);
        if (!deepseek_v4_kv_cache_append(cache, key, position)) {
            free(query); free(q_a); free(q_norm); free(kv_raw); free(key);
            free(context); free(grouped); free(scores);
            return 0;
        }

        int key_count = deepseek_v4_kv_cache_count(cache);
        for (int head = 0; head < heads; head++) {
            const float *head_query = query + head * head_dim;
            float maximum = model->sinks[head];
            for (int index = 0; index < key_count; index++) {
                const float *cached_key = deepseek_v4_kv_cache_key(cache, index);
                float dot = 0.0f;
            ...
            for (int dimension = 0; dimension < head_dim; dimension++) {
                float sum = 0.0f;
                for (int index = 0; index < key_count; index++)
                    sum += scores[index] * deepseek_v4_kv_cache_key(cache, index)[dimension];
                head_output[dimension] = sum / denominator;
            }
            deepseek_v4_attn_rope(head_output, head_dim, model->rope_dim,
        ...
        deepseek_v4_attn_matvec(output + (int64_t)token * hidden, model->o_b,
...
static inline int deepseek_v4_sliding_attention_forward(const DeepSeekV4SlidingAttentionF32 *model,
    ...
    for (int token = 0; token < n_tokens; token++) {
        ...
        for (int head = 0; head < heads; head++) {
            float *row = query + head * head_dim;
            deepseek_v4_attn_rmsnorm(row, row, NULL, head_dim, model->norm_eps);
            deepseek_v4_attn_rope(row, head_dim, model->rope_dim, token, model->rope_theta, 0);
        }
        deepseek_v4_attn_matvec(kv_raw, model->kv, x, head_dim, hidden);
        deepseek_v4_attn_rmsnorm(key, kv_raw, model->kv_norm, head_dim, model->norm_eps);
        deepseek_v4_attn_rope(key, head_dim, model->rope_dim, token, model->rope_theta, 0);
    }

#ifndef DEEPSEEK_V4_COMPRESS_H
#define DEEPSEEK_V4_COMPRESS_H
...
} DeepSeekV4PrefillCompressorF32;

static inline void deepseek_v4_compress_rope(float *vector, int head_dim, int rope_dim,
...
static inline float deepseek_v4_compress_gate(const DeepSeekV4PrefillCompressorF32 *model,
...
static inline float deepseek_v4_compress_value(const DeepSeekV4PrefillCompressorF32 *model,
...
static inline int deepseek_v4_compress_prefill_f32(const DeepSeekV4PrefillCompressorF32 *model,
    ...
    for (int window = 0; window < n_windows; window++) {
        ...
        for (int dimension = 0; dimension < head_dim; dimension++) {
            ...
            for (int slot = 0; slot < slots; slot++) {
                float score = deepseek_v4_compress_gate(model, gate, window, slot, dimension);
            ...
            for (int slot = 0; slot < slots; slot++) {
                ...
                denominator += weight;
                weighted += weight * deepseek_v4_compress_value(model, kv, window, slot, dimension);
            }
        ...
        deepseek_v4_compress_rope(row, head_dim, model->rope_dim,

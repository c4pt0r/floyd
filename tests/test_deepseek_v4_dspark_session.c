...
int main(int argc, char **argv) {
    ...
    CHECK(setenv("DS4_METAL_ENABLE_TINY_BATCH_F16_PAIR", "1", 1) == 0);
    ...
    CHECK(kernel_stats.tiny_batch_shared_gate_up_exact_rows_calls >= 43);
    CHECK(kernel_stats.tiny_batch_shared_down_exact_rows_calls >= 43);
    CHECK(kernel_stats.tiny_batch_router_calls >= 43);
    CHECK(kernel_stats.tiny_batch_attn_hc_calls >= 43);
    CHECK(kernel_stats.tiny_batch_attn_hc_exact_rows_calls >= 43);
    ...
    CHECK(kernel_stats.tiny_batch_f16_pair_calls > 0);
    printf("DeepSeek V4 DSpark tiny batch fusion: moe_calls=%llu "
           ...
           "exact_q8_calls=%llu q8_pair_calls=%llu f16_pair_calls=%llu\n",
           ...
           (unsigned long long)kernel_stats.tiny_batch_shared_gate_up_exact_rows_calls,
           (unsigned long long)kernel_stats.tiny_batch_shared_down_exact_rows_calls,
           (unsigned long long)kernel_stats.tiny_batch_router_calls,
           (unsigned long long)kernel_stats.tiny_batch_attn_hc_calls,
           (unsigned long long)kernel_stats.tiny_batch_attn_hc_exact_rows_calls,
           ...
           (unsigned long long)kernel_stats.tiny_batch_f16_pair_calls);
    ...
    unsetenv("DS4_METAL_ENABLE_TINY_BATCH_F16_PAIR");
    return 0;
}

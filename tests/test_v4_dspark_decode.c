...
int main(int argc, char **argv) {
    ...
    CHECK(v4_real_dspark_state_init(&state));
    CHECK(v4_real_dspark_prefill(&model, prompt_main, 5, &state, &capture));
    CHECK(v4_real_dspark_propose(&model, current_main, 5, (int)input_id,
                                 &state, &capture));
    ...
    printf("v4 DSpark decode: prefill=%.9g stages=%.9g ids=%d/6\n",
           prefill_error, stage_error, hits);
    CHECK(prefill_error < 3e-4f && stage_error < 1e-2f && hits == 6);
    v4_real_dspark_state_free(&state);

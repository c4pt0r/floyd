...
int main(int argc, char **argv) {
    ...
    for (int layer = 0; layer < 3; layer++) {
        for (int site_index = 0; site_index < 2; site_index++) {
            ...
            CHECK(deepseek_v4_hc_forward(&model, input, (int)n_tokens, post, comb, collapsed));
            ...
            printf("DeepSeek V4 hc layer %d %s: post=%.9g comb=%.9g collapsed=%.9g\n",
    ...
    puts("DeepSeek V4 hyper-connection fixture tests: ok");
    return 0;
}

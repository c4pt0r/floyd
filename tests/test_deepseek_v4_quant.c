...
static int test_all_fp4_codes_and_nibble_order(void) {
    ...
    for (int i = 0; i < 16; i++) {
        CHECK(deepseek_v4_fp4_code_value((uint8_t)i) == expected[i]);
        CHECK(deepseek_v4_fp4_packed_value(packed, i) == expected[i]);
    }
    return 0;
}

static int test_e8m0_values(void) {
    CHECK(deepseek_v4_e8m0_to_f32(0) == ldexpf(1.0f, -127));
    CHECK(deepseek_v4_e8m0_to_f32(125) == 0.25f);
    CHECK(deepseek_v4_e8m0_to_f32(126) == 0.5f);
    CHECK(deepseek_v4_e8m0_to_f32(127) == 1.0f);
    CHECK(deepseek_v4_e8m0_to_f32(128) == 2.0f);
    CHECK(deepseek_v4_e8m0_to_f32(254) == ldexpf(1.0f, 127));
    CHECK(isnan(deepseek_v4_e8m0_to_f32(255)));
    return 0;
}
...
static int test_dequant_row_changes_scale_at_32_values(void) {
    ...
    deepseek_v4_fp4_dequant_row(out, packed, scales, 64);
...
static int test_e4m3_values(void) {
    CHECK(deepseek_v4_e4m3_to_f32(0x00) == 0.0f);
    CHECK(deepseek_v4_e4m3_to_f32(0x01) == 0.001953125f);
    CHECK(deepseek_v4_e4m3_to_f32(0x08) == 0.015625f);
    CHECK(deepseek_v4_e4m3_to_f32(0x38) == 1.0f);
    CHECK(deepseek_v4_e4m3_to_f32(0x3c) == 1.5f);
    CHECK(deepseek_v4_e4m3_to_f32(0x7e) == 448.0f);
    CHECK(isnan(deepseek_v4_e4m3_to_f32(0x7f)));
    CHECK(deepseek_v4_e4m3_to_f32(0xb8) == -1.0f);
    return 0;
}
...
static int test_fp8_block_dequant(void) {
    ...
    deepseek_v4_fp8_dequant_matrix(output, weights, scales, 4, 4, 2);
...
int main(void) {
    ...
    puts("DeepSeek V4 quant tests: ok");
    return 0;
}

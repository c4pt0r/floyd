...
static int test_sliding_cache(shards *source) {
    ...
    DeepSeekV4SlidingAttentionF32 model = {
    ...
    DeepSeekV4KVCacheF32 prefill_cache, decode_cache;

    CHECK(deepseek_v4_kv_cache_init(&prefill_cache, DEEPSEEK_V4_KV_CACHE_RING,
                           WINDOW, HD, prefill_storage, prefill_positions));
    CHECK(deepseek_v4_sliding_attention_cached_f32(
        &model, &prefill_cache, input, TOKENS, 0, prefill_output));
    CHECK(deepseek_v4_kv_cache_count(&prefill_cache) == WINDOW);
    CHECK(!deepseek_v4_sliding_attention_cached_f32(
        &model, &prefill_cache, input, 1, TOKENS + 1, prefill_output));

    float retained_keys[WINDOW * HD];
    int64_t retained_positions[WINDOW];
    CHECK(deepseek_v4_kv_cache_copy(&prefill_cache, retained_keys, retained_positions));
    ...
    CHECK(deepseek_v4_kv_cache_init(&decode_cache, DEEPSEEK_V4_KV_CACHE_RING,
    ...
    printf("DeepSeek V4 KV sliding: key=%.9g prefill=%.9g decode=%.9g consistency=%.9g\n",
...
static int test_indexer_append_cache(shards *source) {
    ...
    DeepSeekV4KVCacheF32 cache;

    CHECK(deepseek_v4_kv_cache_init(&cache, DEEPSEEK_V4_KV_CACHE_APPEND,
                           ENTRIES, HD, storage, storage_positions));
    CHECK(deepseek_v4_kv_cache_append(&cache, oracle_keys, 0));
    CHECK(!deepseek_v4_kv_cache_append(&cache, oracle_keys, 0));
    for (int entry = 1; entry < ENTRIES; entry++)
        CHECK(deepseek_v4_kv_cache_append(&cache, oracle_keys + entry * HD, entry * 4));
    CHECK(!deepseek_v4_kv_cache_append(&cache, oracle_keys, ENTRIES * 4));
    CHECK(deepseek_v4_kv_cache_copy(&cache, keys, key_positions));
    for (int entry = 0; entry < ENTRIES; entry++) CHECK(key_positions[entry] == entry * 4);

    DeepSeekV4LightningIndexerF32 model = {
        .n_heads = HEADS, .head_dim = HD, .ratio = 4, .top_k = TOP_K,
    };
    CHECK(deepseek_v4_indexer_forward_f32(&model, queries, keys, weights, query_positions,
    ...
    printf("DeepSeek V4 KV indexer: score=%.9g topk=%d/%d\n",
...
int main(int argc, char **argv) {
    ...
    puts("DeepSeek V4 KV cache fixture tests: ok");
    return 0;
}

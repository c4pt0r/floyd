...
int main(int argc, char **argv) {
    ...
    CHECK(deepseek_v4_indexer_forward_f32(&model, queries, keys, weights, positions,
                                 TOKENS, ENTRIES, actual_scores, actual_indices));
    int64_t negative_position = -1;
    CHECK(!deepseek_v4_indexer_forward_f32(&model, queries, keys, weights, &negative_position,
                                  1, ENTRIES, actual_scores, actual_indices));
    DeepSeekV4LightningIndexerF32 invalid = model;
    invalid.top_k = ENTRIES + 1;
    CHECK(!deepseek_v4_indexer_forward_f32(&invalid, queries, keys, weights, positions,
                                  TOKENS, ENTRIES, actual_scores, actual_indices));
    ...
    printf("DeepSeek V4 lightning indexer: max_abs=%.9g topk=%d/%d\n",
           max_abs, matches, TOKENS * TOP_K);
    CHECK(max_abs < 3e-5f);
    CHECK(matches == TOKENS * TOP_K);
    puts("DeepSeek V4 lightning indexer fixture tests: ok");
    return 0;
}

...
static int run_case(shards *source, const char *mode, int ratio,
    ...
    CHECK(deepseek_v4_compress_prefill_f32(&model, kv, gate, n_tokens, actual));
    CHECK(!deepseek_v4_compress_prefill_f32(&model, kv, gate, n_tokens - 1, actual));
    CHECK(!deepseek_v4_compress_prefill_f32(&model, kv, gate, 0, actual));
    DeepSeekV4PrefillCompressorF32 invalid = model;
    invalid.overlap = 2;
    CHECK(!deepseek_v4_compress_prefill_f32(&invalid, kv, gate, n_tokens, actual));
    ...
    printf("DeepSeek V4 %s compressor: max_abs=%.9g\n", mode, max_abs);
    CHECK(max_abs < 3e-5f);
    return 0;
}
...
int main(int argc, char **argv) {
    ...
    puts("DeepSeek V4 prefill compressor fixture tests: ok");
    return 0;
}

...
int main(int argc, char **argv) {
    ...
    CHECK(deepseek_v4_forward_forward(&model, 3, &capture));
...
#undef ACCUM_ERROR
    ...
    printf("DeepSeek V4 real layer0: attn=%.9g ffn=%.9g router=%.9g weights=%.9g routes=%d/%d\n",
    ...
    CHECK(layer2_input && deepseek_v4_forward_layer2_csa_forward(&model, layer2_input, &csa));
    ...
    printf("DeepSeek V4 real layer2 CSA: compressor=%.9g indexer=%.9g indices=%d/4 bias=%d/4\n",
    ...
    printf("DeepSeek V4 real layers 0-2: layer0=%.9g layer1=%.9g layer2=%.9g\n",
    ...
    DeepSeekV4ForwardLayerState hca_state;
    DeepSeekV4ForwardLayerCapture hca_capture = {
    ...
    CHECK(deepseek_v4_forward_layer_state_init(&hca_state, 3, HCA_TOKENS));
    CHECK(deepseek_v4_forward_layer_forward_state(&model, 3, hca_token_ids, HCA_TOKENS,
    ...
    float hca_gate_error = max_error(hca_projected_gate, expected_hca_gate,
                                     HCA_TOKENS * 512);
    float hca_compress_error = max_error(deepseek_v4_kv_cache_key(&hca_state.compressed, 0),
    ...
    printf("DeepSeek V4 real layer3 HCA: kv=%.9g gate=%.9g compress=%.9g router=%.9g weights=%.9g routes=%d/%d output=%.9g\n",
    ...
    deepseek_v4_forward_layer_state_free(&hca_state);
    ...
    DeepSeekV4ForwardLayerCapture transition_capture = {
        .index_scores = transition_scores,
        .index_ids = transition_ids,
        .block_bias = transition_bias,
        .router_indices = transition_routes,
    };
    DeepSeekV4ForwardLayerState transition_hca, transition_csa;
    CHECK(transition_streams);
    CHECK(deepseek_v4_forward_layer_state_init(&transition_hca, 3, 4));
    CHECK(deepseek_v4_forward_layer_forward_state(&model, 3, token_ids, 4,
    ...
    float transition_layer3_error = max_error(transition_streams, expected_layer3,
                                              4 * HC * D);
    CHECK(deepseek_v4_kv_cache_count(&transition_hca.compressed) == 0);
    CHECK(transition_hca.next_position == 4);
    CHECK(deepseek_v4_forward_layer_state_init(&transition_csa, 4, 4));
    CHECK(deepseek_v4_forward_layer_forward_state(&model, 4, token_ids, 4,
    ...
    float transition_compress_error = max_error(
        deepseek_v4_kv_cache_key(&transition_csa.compressed, 0), expected_layer4_kv, 512);
    ...
    printf("DeepSeek V4 real layers 3-4: layer3=%.9g layer4=%.9g compress=%.9g indexer=%.9g indices=%d/4 bias=%d/4 routes=%d/24\n",
    ...
    deepseek_v4_forward_layer_state_free(&transition_hca);
    deepseek_v4_forward_layer_state_free(&transition_csa);
    ...
    DeepSeekV4ForwardModelState base_state;
    ...
    CHECK(deepseek_v4_forward_model_state_init(&base_state, 4));
    CHECK(deepseek_v4_forward_base_layers_forward(&model, token_ids, 4, &base_state,
    ...
    printf("DeepSeek V4 real base layers: l4=%.9g l20=%.9g l40=%.9g l41=%.9g l42=%.9g targets=%.9g order_miss=%d set_miss=%d\n",
    ...
    CHECK(deepseek_v4_forward_base_head_forward(&model, head_input, 4, &head_capture));
    ...
    CHECK(deepseek_v4_forward_base_head_forward(&model, base_streams, 4, &head_capture));
    int continuous_argmax = 0;
    for (int token = 1; token < VOCAB; token++)
        if (head_logits[token] > head_logits[continuous_argmax]) continuous_argmax = token;
    printf("DeepSeek V4 real base head: hidden=%.9g norm=%.9g logits=%.9g isolated=%d expected=%lld continuous=%d\n",
    ...
    DeepSeekV4ForwardDSparkCapture dspark_capture = {
    ...
    CHECK(deepseek_v4_forward_dspark_state_init(&dspark_state));
    CHECK(deepseek_v4_forward_dspark_forward(&model, dspark_main_x, 4,
    ...
    printf("DeepSeek V4 real DSpark: prefill=%.9g stage0=%.9g stage1=%.9g stage2=%.9g ids=%d/6 confidence=%.9g\n",
    ...
    deepseek_v4_forward_dspark_state_free(&dspark_state);
    ...
    deepseek_v4_forward_model_state_free(&base_state);
    free(base_streams); free(base_checkpoints); free(base_targets); free(base_routes);
    puts("DeepSeek V4 real layer0 tests: ok");
    return 0;
}

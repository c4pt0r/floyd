...
int main(int argc, char **argv) {
    ...
    CHECK(v4_runtime_init(&runtime, argv[1], 5));
    CHECK(v4_runtime_forward(&runtime, prefill_ids, 4, &logits));
    CHECK(v4_runtime_forward(&runtime, &decode_id, 1, &logits));
    const float *decode = v4_runtime_last_streams(&runtime);

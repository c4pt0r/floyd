...
static int test_official_fp8(shards *source, float *max_abs, int *top1_matches) {
    ...
    deepseek_v4_fp8_dequant_matrix(dequant, weight, scales, ROWS, COLUMNS, BLOCK);
    reference_matmul(reference, input, dequant, INPUTS, ROWS, COLUMNS);
    CHECK(deepseek_v4_fp8_matmul_f32(actual, input, INPUTS, weight, scales,
                            ROWS, COLUMNS, BLOCK));
...
static int test_official_fp4(shards *source, float *max_abs, int *top1_matches) {
    ...
    for (int row = 0; row < ROWS; row++) {
        ...
        deepseek_v4_fp4_dequant_row(dequant + row * COLUMNS,
    ...
    make_inputs(input, INPUTS, COLUMNS);
    reference_matmul(reference, input, dequant, INPUTS, ROWS, COLUMNS);
    CHECK(deepseek_v4_fp4_matmul_f32(actual, input, INPUTS, weight, scales,
                            ROWS, COLUMNS));
    *max_abs = max_error(actual, reference, INPUTS * ROWS);
    *top1_matches = 0;
    for (int i = 0; i < INPUTS; i++)
...
int main(int argc, char **argv) {
...
#ifdef FLOYD_METAL
    CHECK(deepseek_v4_quant_backend_enable_metal(4));
    deepseek_v4_quant_backend_reset_stats();
#endif
    ...
    printf("DeepSeek V4 native FP4: max_abs=%.9g top1=%d/4\n", fp4_error, fp4_top1);
    printf("DeepSeek V4 native FP8: max_abs=%.9g top1=%d/4\n", fp8_error, fp8_top1);
    CHECK(fp4_error < 3e-5f && fp4_top1 == 4);
    CHECK(fp8_error < 3e-5f && fp8_top1 == 4);
#ifdef FLOYD_METAL
    DeepSeekV4QuantBackendStats stats = deepseek_v4_quant_backend_stats();
    printf("DeepSeek V4 native Metal: calls=%llu cpu_fallbacks=%llu\n",
...
#endif
    puts("DeepSeek V4 native quantized matmul tests: ok");
    return 0;
}

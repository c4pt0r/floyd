...
int main(int argc, char **argv) {
    ...
    CHECK(deepseek_v4_sliding_attention_forward(&model, input, 0, actual));
    CHECK(deepseek_v4_sliding_attention_forward(&model, input, (int)n_tokens, actual));
    ...
    printf("DeepSeek V4 sliding attention: max_abs=%.9g\n", max_abs);
    CHECK(max_abs < 3e-5f);
    puts("DeepSeek V4 sliding attention fixture tests: ok");
    return 0;
}

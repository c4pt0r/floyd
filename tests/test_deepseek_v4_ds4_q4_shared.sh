...
q4_model=$3
tokens=${NGEN:-4}
tmp=$(mktemp -d -t floyd-deepseek-v4-ds4-q4-shared.XXXXXX)
...
run_model q4_shared "$q4_model"
diff -u "$tmp/q8.tokens" "$tmp/q4_shared.tokens"

q8_perf=$(grep 'DEEPSEEK_V4_PERF ' "$tmp/q8.stderr" | tail -1)
q4_perf=$(grep 'DEEPSEEK_V4_PERF ' "$tmp/q4_shared.stderr" | tail -1)
printf '%s\n%s\n' "$q8_perf" "$q4_perf"
printf 'DeepSeek V4 DS4 Q4 shared parity: %s/%s tokens\n' "$tokens" "$tokens"

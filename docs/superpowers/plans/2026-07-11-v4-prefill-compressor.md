...
- Update `tests/test_make_v4_oracle.py`; run `make test-v4-oracle` first to observe RED, then GREEN.
- Commit the oracle change without generated artifacts.

## Task 2: C Fixture and Kernel

- Add `tests/test_v4_compress_fixture.c` and a `test-v4-compress` Make target.
- Compile first and confirm RED because `v4_compress.h` is missing.
- Implement validation, HCA/CSA window layout, stable per-dimension softmax pooling, weighted RMSNorm, and compressed-position interleaved RoPE in `v4_compress.h`.

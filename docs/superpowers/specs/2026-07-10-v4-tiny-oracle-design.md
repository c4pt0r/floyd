...
`tools/make_v4_oracle.py` builds a three-layer `DeepseekV4ForCausalLM` with hidden size 64, two mHC streams, eight routed experts, top-2 routing, and a 128-token vocabulary. Fixed seed 1234 initializes weights. Hash-router tables contain deterministic token-dependent expert pairs rather than the all-zero initialization used by Transformers.

The default output directory is `fixture_tiny_v4`, which is already covered by the repository's `fixture_tiny*` ignore rule. It contains `config.json`, `model.safetensors`, and `oracle.safetensors`. The separate JSON reference records prompt IDs, greedy full IDs, and per-position argmax predictions.

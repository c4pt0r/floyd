...
`deepseek_v4_indexer.h` consumes these precomputed F32 boundaries. For each query/key/head it computes `ReLU(q dot k) / sqrt(head_dim)`, multiplies by the projected head weight divided by `sqrt(n_heads)`, and sums across heads. Causal selection permits entry `e` only when `e < (position + 1) / ratio`; unavailable result slots are `-1`.

...
builds. The old standalone `deepseek_v4_chat` executable is removed; `floyd` is the
...
Rename all DeepSeek V4-specific source, test, and oracle files from `deepseek_v4_*` to
`deepseek_v4_*`. Rename C symbols and types from `deepseek_v4_*`/`V4*` to
`deepseek_v4_*`/`DeepSeekV4*`, Make targets from `test-deepseek-v4-*` to
`test-deepseek-v4-*`, and user-facing environment/log prefixes from `DEEPSEEK_V4_*` to

...
test-deepseek-v4-ds4-q4-shared: floyd
	@test -n "$(DSPARK)" || (echo "set DSPARK=/path/to/DeepSeek-V4-Flash-DSpark"; exit 2)
	@test -n "$(DS4_GGUF)" || (echo "set DS4_GGUF=/path/to/q8-base.gguf"; exit 2)
	@test -n "$(DS4_Q4_SHARED_GGUF)" || (echo "set DS4_Q4_SHARED_GGUF=/path/to/q4-shared.gguf"; exit 2)
	sh tests/test_deepseek_v4_ds4_q4_shared.sh "$(DSPARK)" "$(DS4_GGUF)" "$(DS4_Q4_SHARED_GGUF)"

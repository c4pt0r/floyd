...
tests/test_deepseek-v4-dspark-support: tests/test_deepseek_v4_dspark_support.c $(DS4_CORE_OBJS)
	$(CC) -O2 -I$(DS4_DIR) $< $(DS4_CORE_OBJS) -o $@ -lm -pthread \
	  -framework Accelerate -framework Metal -framework MetalKit -framework Foundation

test-deepseek-v4-dspark-support: tests/test_deepseek-v4-dspark-support
	@test -n "$(DSPARK_SUPPORT)" || (echo "set DSPARK_SUPPORT=/path/to/three-stage-dspark.gguf"; exit 2)
	./tests/test_deepseek-v4-dspark-support "$(DSPARK_SUPPORT)"
